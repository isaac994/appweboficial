...
<body>
    ...
    <div class="info-section">
        <h3 style="text-align: center; margin-bottom: 20px;">Detalle Completo de Productos Comprados</h3>
        <table class="productos-table">
            <thead>
                <tr>
                    ...
                    <th>Marca/Modelo - Descripción</th>
            ...
            <tbody>
                ...
                @foreach($compras as $compra)
                    @foreach($compra->detalles_completos as $detalle)
                    <tr>
                        <td class="text-center">{{ $contador++ }}</td>
                        <td>{{ date('d/m/Y', strtotime($compra->fecha)) }}</td>
                        <td>{{ $detalle->proveedor_nombre ?? 'Sin proveedor' }}</td>
    ...
    <div class="info-section">
        <h3 style="text-align: center; margin-bottom: 20px;">Resumen de Compras por Proveedor</h3>
        <table class="productos-table">
            <thead>
                <tr>
                    <th class="text-center">Nro</th>
            ...
            <tbody>
                @php
                    $comprasPorProveedor = $compras->groupBy('id_proveedor');
                    $contador = 1;
                @endphp
                @foreach($comprasPorProveedor as $proveedorId => $comprasDelProveedor)
                @php
                    $proveedor = $comprasDelProveedor->first()->proveedor;
                    $totalProductos = $comprasDelProveedor->sum(function($compra) { return $compra->detalles_completos->sum('cantidad'); });
                    $totalCompra = $comprasDelProveedor->sum('total');
                @endphp
                <tr>
                    <td class="text-center">{{ $contador++ }}</td>
                    <td>{{ $proveedor->nombre ?? 'Sin proveedor' }}</td>
                    <td class="text-center">{{ $totalProductos }}</td>
                    <td class="text-right font-bold">Bs. {{ number_format($totalCompra, 2, ',', '.') }}</td>
                </tr>
                @endforeach
                <tr style="background-color: #e3f2fd; font-weight: bold; border-top: 2px solid #1976d2;">
                    ...
                    <td class="text-center">{{ $compras->sum(function($compra) { return $compra->detalles_completos->sum('cantidad'); }) }}</td>
                    <td class="text-right">Bs. {{ number_format($totalCompras, 2, ',', '.') }}</td>
                </tr>
            </tbody>
        </table>
    </div>
