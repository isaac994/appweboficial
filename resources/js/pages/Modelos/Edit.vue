    ...
    <div class="min-h-screen bg-gradient-to-br from-[#0a1628] via-[#0d1b2e] to-[#0a1628]">
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex items-center justify-between mb-8">
          <div>
            <h1 class="text-3xl font-bold text-white mb-2">Editar Modelo</h1>
            <p class="text-purple-300">Modifica la información del modelo</p>
          </div>
          <Link
          ...
          <form @submit.prevent="submitForm" class="space-y-6">
            ...
            <div class="flex justify-end space-x-4 pt-6">
              ...
              >
                ...
                <span v-else>Actualizar Modelo</span>
              </button>
            </div>
          </form>
        </div>
...
const form = ref({
  ...
  id_marca: props.modelo.id_marca.toString()
})

const submitForm = () => {
  loading.value = true

  router.put(route('modelos.update', props.modelo.id_modelo), form.value, {
    onSuccess: () => {
      // El mensaje de éxito se mostrará automáticamente a través del sistema de flash de Inertia
    },
    onError: () => {
      loading.value = false
    },
